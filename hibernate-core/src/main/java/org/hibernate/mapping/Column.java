...
public class Column implements Selectable, Serializable, Cloneable {
    ...
    private Default defaultValueAnnotation;
    ...
    public void setName(String name) {
        if (name.charAt(0) == '`' || Dialect.QUOTE.indexOf(name.charAt(0)) > -1 //TODO: deprecated, remove eventually
                ) {
            quoted = true;
    ...
    public int getSqlTypeCode(Mapping mapping) throws MappingException {
        ...
        try {
            ...
            throw new MappingException("Could not determine type for column " + name + " of type " + type.getClass()
                    .getName() + ": " + e.getClass().getName(), e);
        }
    }
    ...
    public String getSqlType(Dialect dialect, Mapping mapping) throws HibernateException {
        return sqlType == null ? dialect.getTypeName(getSqlTypeCode(mapping), getLength(), getPrecision(), getScale()) : sqlType;
    }
    ...
    public String getTemplate(Dialect dialect, SQLFunctionRegistry functionRegistry) {
        return hasCustomRead() ? Template.renderWhereStringTemplate(customRead, dialect, functionRegistry) : Template.TEMPLATE + '.' + getQuotedName(dialect);
    }
    ...
    public String getDialectSpecificDefaultValue(Dialect dialect) {
        if (defaultValueAnnotation == null) {
            return defaultValue;
        } else {
            try {
                return getDefaultValueFromAnnotation(dialect);
            } catch (Exception e) {
                throw new MappingException("Can not determine default value for column " + name + " of type " + getValue()
                        .getType()
                        .getClass()
                        .getName(), e);
            }
    ...
    private String getDefaultValueFromAnnotation(Dialect dialect) throws Exception {
        if (defaultValueAnnotation.stringValue().length > 0) {
    ...
    private <T> String convertDefaultValue(Dialect dialect, T value) throws Exception {
        @SuppressWarnings({"unchecked"}) LiteralType<T> type = (LiteralType<T>) getValue().getType();
        return type.objectToSQLString(value, dialect);
    }
}
